...
export default {
    ...
    mounted() {
        ...
        if (this.$route.query.openid) {
            localStorage.setItem('operatorcode', this.$route.query.openid)
        }
        ...
        getUnitInfo({ businesstype: "FirstPageFunc", code: this.operatorcode, id: location.href }).then(res => {
    ...
    methods: {
        ...
        handleViewReport() {
            ...
            this.$router.push({
                path: "/viewReport",
                query: { printFlag: "个人报告书", regid: CurrentPerson.regid },
            });
            this.dialogVisible = false;
        },
        ...
        async View_report() {
            let CurrentPerson = JSON.parse(localStorage.getItem("currentData")) || "";
            this.$router.push({ path: "/viewReport", query: { regid: CurrentPerson.regid } });
            return;
        ...
        // 跳转到预约界面
        jumpReservation(flag) {
            if (!this.operatorcode) {
                this.$router.push("/login");
                return false;
            }
            // 设置体检类型 personalflag: 0, // 0-个人  1-单位 funcstatus
        ...
        // 体检推荐
        jumpGroupsRecommend(flag) {
            this.$toast.fail("未配置健康问卷字典!");
        },
        // 跳转集合回调
        handleButtonAll(codeindex) {
            switch (codeindex) {
                case '1':
                    this.jumpReservation()
                    break;
                case '2':
                    this.jumpReservation()
                    break;
